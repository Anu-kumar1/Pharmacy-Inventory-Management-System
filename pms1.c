...
int reorderlvl, int supid, char supname[], int supquan, char phn[], int medNo[])
{
    ...
    if(found)  //update
    {
        ...
        int oldquan=ptr->inventory.Quantity;
        for (int j = oldquan; j < oldquan+quant; j++)
        {
            if (!medNo) {
...
{
    int found = 0;MedNode*ptr=head,*prev=NULL;
    if(head==NULL){
...
SupplierInfoNode* AllrounderSupplier(SupplierInfoNode* suphead)
{
    ...
    while(supptr && count<5)
    {
        printf("%d) %s  %d\n",count+1, supptr->sup.sup_name,supptr->sup.UniqueQuant);
        supptr=supptr->next;
        count++;
    }
    return suphead;
